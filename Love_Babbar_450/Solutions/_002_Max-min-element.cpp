...
struct Pair getMinMax(int arr[], int low, int high){
    ...
    if(minmaxLeft.min < minmaxRight.min)    minmax.min = minmaxLeft.min;
    else minmax.min = minmaxRight.min;
    if(minmaxLeft.max > minmaxRight.max)    minmax.max = minmaxLeft.max;
    else minmax.max = minmaxRight.max;
    return minmax;
}
